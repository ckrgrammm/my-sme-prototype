...
export default function WorkflowScreen({ lang = 'en', industry, onToast }) {
  ...
  useEffect(() => { api.getWorkflow(industry).then(setWorkflow).catch((error) => onToast(error.message)); }, [industry]);
  ...
  return (
    <div className="dashboard-shell">
      ...
      <section className="rounded-2xl border border-amber-200 bg-amber-50/60 p-4 md:p-5">
        ...
        {exceptions.length ? <div className="grid gap-3 lg:grid-cols-3">{exceptions.map((item) => <article key={item.id} className="rounded-xl border border-amber-200 bg-white p-4"><div className="flex items-center justify-between"><strong className="text-xs text-primary">{item.id}</strong><span className="text-[10px] font-bold text-muted-foreground">{item.age}</span></div><h3 className="mt-2 text-sm font-extrabold">{item.customer}</h3><p className="mt-1 text-xs text-muted-foreground">{pick(lang, item.automation)}</p><Button className="mt-3 w-full" size="sm" onClick={() => advance(item)} disabled={busy === item.id}>{item.needsApproval ? c.approve : c.fix}</Button></article>)}</div> : <p className="text-sm text-green-700">{c.allClear}</p>}
      </section>
